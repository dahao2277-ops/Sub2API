...
const encodeCsvCell = (value: unknown): string => {
  const raw = value == null ? '' : String(value)
  // Prevent spreadsheet applications from interpreting exported user-controlled
  // values as formulas while preserving their visible content.
...
const exportToExcel = async () => {
  ...
  try {
    ...
    if(!c.signal.aborted) {
      const csv = csvRows.map(row => row.map(encodeCsvCell).join(',')).join('\r\n')
      saveAs(
